...
import { computeLayout, type Column, type Layout } from './layout.ts';
import { displayWidth, truncate, fixedWidth, type Align } from './width.ts';
...
export type Row<T> =
   ...
   */
  | { type: 'literal'; render: (totalWidth: number) => string };
...
function padCell(cell: string, width: number, align: Align | undefined, minPad: number): string {
 ...
 * independently).
 */
function measure<T>(columns: Column<T>[], dataRows: Array<{ row: T; indent?: string }>): Map<string, number> {
...
export function renderTable<T>(
  ...
  const dataRows = rows.filter((r): r is { type: 'data'; row: T; indent?: string } => r.type === 'data');
  ...
  const isTrimmed = (key: string) => key === trimmableKey && key === lastVisibleKey;
  ...
  for (const r of rows) {
    ...
    for (const col of visibleCols) {
      ...
      if (col.key === firstKey) allotted -= margin;
      if (col.key === flexKey) allotted -= indentW;
