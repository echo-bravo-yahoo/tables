...
// src/layout.ts
function computeLayout(columns, contentWidths, width, opts) {
  const margin = opts?.margin ?? 0;
  const maxOf = (col) => col.max ?? Infinity;
  let active = columns.map((c, i) => ({ c, i })).sort((a, b) => a.c.priority - b.c.priority || a.i - b.i).map((x) => x.c);
...
function measure(columns, dataRows) {
  const contentWidths = /* @__PURE__ */ new Map();
...
function renderTable(columns, rows, options) {
  ...
  const trimmableKey = trimLast ? columns[columns.length - 1]?.key : void 0;
...
// src/hyperlink.ts
function hyperlink(url, text, opts) {
